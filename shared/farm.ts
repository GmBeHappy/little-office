...
export const FARM_POND = { x: 110, y: 420, w: 250, h: 100 };
const hayBales = [
...
export function drawFarmMap(
  ...
  const ink = "#3e5233";
  rect(0, 0, 1120, 720, 0x65836b);
  rect(32, 40, 1060, 644, 0x92a984);
  rect(40, 56, 1040, 620, 0xb4c59b);
  for (let i = 0; i < 120; i++) {
    const x = 48 + ((i * 137) % 1024),
      y = 64 + ((i * 89) % 600);
    rect(x, y, 6, 2, i % 3 ? 0xa2b78f : 0xc6d3ad);
  }
  // Dirt paths link both room doors, the yard gate, the pond, and the field.
  rect(178, 300, 540, 44, 0xdfcfaa);
  rect(700, 300, 80, 340, 0xdfcfaa);
  rect(178, 284, 112, 20, 0xdfcfaa);
  rect(868, 284, 102, 60, 0xdfcfaa);
  rect(620, 344, 46, 160, 0xdfcfaa);
  rect(596, 460, 70, 40, 0xdfcfaa);
  for (let x = 190; x < 700; x += 54) {
    rect(x, 306, 30, 3, 0xc4b38e);
    rect(x + 26, 328, 22, 3, 0xc4b38e);
  }
  // A pale loop connects the orchard, fishing dock and crop garden.
  rect(84, 350, 286, 64, 0xdfcfaa);
  rect(304, 380, 64, 108, 0xdfcfaa);
  rect(610, 606, 244, 34, 0xdfcfaa);
  rect(780, 322, 222, 34, 0xdfcfaa);
  rect(810, 350, 34, 274, 0xdfcfaa);
  // Soft meadow islands break up the old rectangular lawn.
  rect(96, 572, 210, 66, 0xa2b78f);
  rect(376, 80, 120, 124, 0xa2b78f);
  for (let i = 0; i < 18; i++) {
    const x = 100 + ((i * 37) % 172),
      y = 430 + ((i * 13) % 28);
    rect(x, y, 3, 7, 0x839b76);
    rect(x - 2, y, 7, 3, i % 2 ? 0xefe3b8 : 0xd3b5b2);
  }
  // The farmhouse: cream walls under a red-shingled roof.
  rect(38, 30, 344, 34, 0xb37b66);
  rect(46, 42, 328, 12, 0x906756);
  for (let x = 44; x < 378; x += 22) rect(x, 32, 14, 6, 0xca9780);
  ...
  for (const wall of barnWalls) {
    rect(wall.x, wall.y, wall.w, wall.h, 0xac7966);
    rect(wall.x, wall.y, wall.w, 3, 0xca9780);
  }
  rect(872, 262, 92, 20, 0xf2e6d0);
  rect(884, 266, 68, 16, 0x8a5f3c);
  rect(884, 266, 68, 3, 0x6b4526);
  for (const b of barnHay) {
    rect(b.x, b.y, b.w, b.h, 0xbea67b);
    rect(b.x + 4, b.y + 4, b.w - 8, b.h - 10, 0xdac593);
  ...
  rect(552, 78, 32, 100, 0xdfcfaa);
  rect(552, 78, 32, 8, 0xa5814f);
  rect(560, 150, 16, 22, 0x7d5334);
  rect(548, 56, 40, 8, 0xac7966);
  ...
  // Move the fishing terrace north; the exported pond matches its new bank.
  const pondRect = rect;
  {
    const rect = (x: number, y: number, w: number, h: number, color: number) =>
      pondRect(x, y - 80, w, h, color);
    const pondEffect: MapEffectSink | undefined = effect
      ? (item) => effect({ ...item, y: item.y - 80 })
      : undefined;
    // The pond: cool water, lily pads, cattails, and a small fishing dock.
    rect(102, 492, 266, 116, 0x7da69b);
    rect(110, 500, 250, 100, 0x98bdb0);
    rect(126, 514, 218, 72, 0x7da69b);
    for (let i = 0; i < 7; i++)
      waterRipple(
        rect,
        pondEffect,
  ...
  for (const b of hayBales) {
    rect(b.x, b.y, b.w, b.h, 0xbea67b);
    rect(b.x + 5, b.y + 5, b.w - 10, b.h - 10, 0xdac593);
