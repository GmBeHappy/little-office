...
test.skip(
  !accounts.length,
  "Run through bun scripts/e2e.ts zen-garden.spec.ts",
);

test("Sakura Garden can be selected and both relocated tea houses can be clicked", async ({
  page,
}) => {
  ...
  try {
    ...
    await page
      .getByRole("button", { name: "Sakura Garden map", exact: true })
      .click();
    await page
      .locator(".modal")
      .screenshot({ path: "test-results/zen-picker.png" });
    if (original.mapId !== "zen-small")
      await page
    ...
    await expect(canvas).toBeVisible();
    await page.screenshot({ path: "test-results/zen-desktop.png" });
    for (const zone of mapZones(getMap("zen-small")).filter(
      (z) => z.id !== "floor",
    )) {
      ...
      await canvas.click({
        position: {
          x: (zone.x + 240 - scrollX) * zoom,
          y: (zone.y + 175 - scrollY) * zoom,
        },
      });
      await expect
        .poll(() => snapshot?.people.find((p) => p.id === snapshot?.self)?.zone)
        .toBe(zone.id);
      expect(
    ...
    await expect(
      page.getByRole("button", { name: "แผนที่สวนซากุระ", exact: true }),
    ).toBeVisible();
    await page.setViewportSize({ width: 390, height: 844 });
    await page.screenshot({ path: "test-results/zen-mobile-thai.png" });
    expect(
      await page.evaluate(
